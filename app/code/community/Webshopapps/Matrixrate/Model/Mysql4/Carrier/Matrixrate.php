...
/*
    	...
    	foreach ($request->getAllItems() as $item) {
    		...
    		$request->setData($request->getConditionName(),$item_weight);
    		$bind = array(
    		...
    		$tablerateModel=Mage::getModel("Mage_Shipping_Model_Resource_Carrier_Tablerate");
    		...
    		if (is_array($request->getConditionName())) {
    			...
    			//echo $request->getConditionName().'    '.$request->getData($request->getConditionName());die;
    			$select->where('condition_name = :condition_name');
    			$select->where('condition_value <= :condition_value');
    		}
    	
    		$result=$adapter->fetchRow($select, $bind);
    		$condition_value=($result['condition_value']*$item->getQty())+$condition_value;
    		$price=($result['price']*$item->getQty())+$price;
    		$cost=($result['cost']*$item->getQty())+$cost;
    	}
    	if($condition_value==0 && $price==0 && $cost==0 )
    		return array();
    	$result['condition_value']=$condition_value;
    	$result['price']=$price;
    	$result['cost']=$cost;
    	$request->setData($request->getConditionName(),$oldweight);
