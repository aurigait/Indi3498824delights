...
class Magehouse_Slider_Block_Catalog_Layer_Filter_Price extends Mage_Catalog_Block_Layer_Filter_Price 
{
	...
	*/
	public function __construct(){
		...
		$this->_currentCurrencyCode = Mage::app()->getStore()->getCurrentCurrencyCode();
		$this->setMinPrice();
		$this->setMaxPrice();
	...
	*/
	...
	public function setNewPrices(){
		...
		$a[0][] = 'ASC';
		$loadedCollection = $this->getLayout()->getBlockSingleton('catalog/product_list')->getLoadedProductCollection()->setOrder('min_price','DESC')->getSelect()->limit("")->setPart('order',$a)->query()->fetchAll();
		...
		if(count($loadedCollection) > 0){
			$loadedMin = $this->convertBaseCurrent($loadedCollection[0]['min_price']);
			$loadedMax = $this->convertBaseCurrent($loadedCollection[$tot-1]['min_price']);	
		}	
		...
		if($this->isTextBoxEnabled()){
			$html = '
				<div class="text-box">
					...
					<button type="submit" class="button go" style="'.$goBtnStyle.'"><span><span>'.$this->getGoBtnText().'</span></span></button>		
	...
	*/
	...
	public function prepareCustomStyles(){
		...
		$html = '<style type="text/css">';	
			...
			$html .= 'height:'.$handleHeight.'px; border:none;}'; //width:'.$handleWidth.'px;
			
			$html .= '.ui-slider{';
			if($bgSlider){$html .= 'background:'.$bgSlider.';';}
			$html .= ' height:'.$sliderHeight.'px; border:none;}'; //width:'.$handleWidth.'px;
	...
	*/
	public function setMinPrice(){
		...
		if( (isset($_GET['q']) && !isset($_GET['min'])) || !isset($_GET['q'])){
			...
			$this->_minPrice = $this->convertBaseCurrent($this->_searchSession->getMinPrice());	
		}
		
		$this->convertBaseCurrent($this->_minPrice);
	}
	...
	*/
	public function setMaxPrice(){
		...
		$this->convertBaseCurrent($this->_maxPrice);
	...
	*/
	public function setProductCollection(){
		...
		if($this->_currentCategory){
			...
			$this->_productCollection = Mage::getSingleton('catalogsearch/layer')->getCurrentCategory()->getProductCollection()	
							->addAttributeToSelect('*')
							->setOrder('price', 'ASC');
		}			
	...
	*/
	...
	public function convertBaseCurrent($price)
	{
		return Mage::helper('directory')->currencyConvert($price, $this->_baseCurrencyCode, $this->_currentCurrencyCode);
